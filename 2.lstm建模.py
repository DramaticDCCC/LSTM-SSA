...
np.random.seed(0)
# In[] 加载数据
xlsfile=pd.read_excel('C:/Users/LENOVO/Desktop/小论文20%数据.xlsx').iloc[0:,2:]# 前两列的日期不作为特征之一
data=np.array(xlsfile)
# 拟用前一时期的 7 个特征与预测时期的aqi作为输入，预测时期的 6 种空气污染物浓度作为输出
in_=np.hstack((data[1:,0:1],data[0:-1,:]))
out_=data[1:,0:]
#划分数据，一共1740个样本 选择1571个样本作为训练集 24个测试集
n=range(in_.shape[0])
m=200927 # 1740个数据，选出24X7小时数据为测试集
...
alpha = 0.01# 学习率
num_epochs = 88#迭代次数
hidden_nodes0 =60#第一隐含层神经元
hidden_nodes = 47#第二隐含层神经元
...
savemat('D:\建模\结果\lstm_result.mat',{'true':test_label,'pred':test_pred})
# In[]计算各种指标
# mape
...
mean_observed = np.mean(test_label)
nse = 1 - (np.sum((np.array(test_pred) - np.array(test_label))**2) / np.sum((np.array(test_label) - mean_observed)**2))

print("The Nash-Sutcliffe Efficiency (NSE) is:", nse)

test_pred.to_csv("C:/Users/LENOVO/Desktop/test_pred_LSTM-测试.csv")